...
from core.finetune.utils import (
    ...
    unload_model,
    unwrap_model,
    load_lora_weights_with_conversion,
)
...
class Trainer:
    ...
    def prepare_trainable_parameters(self):
        ...
        for attr_name, component in vars(self.components).items():
            if hasattr(component, "requires_grad_"):
                if self.args.training_type == "sft" and attr_name == "transformer":
                    ...
                    component.requires_grad_(False) # everything in model cls will be freezed
        ...
        if self.args.training_type == "lora":
            ...
            # load learnable embeddings:
            learnable_domain_embeddings = torch.load(os.path.join(self.args.lora_path, "learnable_domain_embeddings.pt"), weights_only=True)
            self.components.transformer.learnable_domain_embeddings.data = learnable_domain_embeddings.to(self.components.transformer.device, self.components.transformer.dtype)
            # add lora and load 4DNeX lora weights (with general 4DNeX prior):
            self.components.transformer.add_adapter(transformer_lora_config) # grad is True for lora parameters
            ori_state_dict = self.components.transformer.state_dict()
            lora_state_dict = safetensors.torch.load_file(os.path.join(self.args.lora_path,"pytorch_lora_weights.safetensors"))
            updated_state_dict = load_lora_weights_with_conversion(lora_state_dict, ori_state_dict)
            logger.info(f"Successfully matching and loading LoRA weights")
            """
            # load pretrained other weights: (only for resume training when you only have converted ckpt, otherwise, directly resume deepspeed ckpt)
            added_dict = safetensors.torch.load_file(os.path.join(self.args.lora_path,"added_weights.safetensors"))
            for k, v in added_dict.items():
                updated_state_dict[k] = v.to(self.components.transformer.device, self.components.transformer.dtype)
                logger.info(f"Loaded {k}")
            """
            self.components.transformer.load_state_dict(updated_state_dict, strict=False)
            ...
            for name, param in self.components.transformer.named_parameters():
                if 'learnable_domain_embeddings' in name or 'action_encoder' in name or 'arm' in name: # trainable parameters besides lora, which is previously freezed at L234, here need to open again
                    param.requires_grad_(True)
                    logger.info(f"Training {name} after adding LoRA")
                # TODO: freeze pretrained lora if you only want to train added modules:
                # if 'lora' in name:
                #     param.requires_grad_(False)
                #     logger.info(f"Freeze {name}, and only train added modules")
    ...
    def prepare_optimizer(self) -> None:
        ...
        # num_update_steps_per_epoch = math.ceil(len(self.data_loader) / (self.args.gradient_accumulation_steps * self.args.batch_size * self.accelerator.num_processes))
    ...
    def clean_config_for_tracker(self, config_dict):
        clean_config = {}
        
        for key, value in config_dict.items():
            if value is None:
                clean_config[key] = "None"
                logger.info("Cleaned None config")
            elif isinstance(value, (int, float, str, bool)):
                clean_config[key] = value
            elif isinstance(value, list):
                if all(isinstance(x, (int, float)) for x in value):
                    if value:
                        clean_config[f"{key}_mean"] = float(sum(value) / len(value))
                        clean_config[f"{key}_min"] = float(min(value))
                        clean_config[f"{key}_max"] = float(max(value))
                    else:
                        clean_config[f"{key}_empty"] = True
                else:
                    clean_config[key] = str(value)
                logger.info("Cleaned List config")
                ...
                for k, v in value.items():
                    if isinstance(v, (int, float, str, bool)):
                        simple_dict[k] = v
                if simple_dict:
                    clean_config[key] = str(simple_dict)
                else:
                    clean_config[key] = str(value)
                logger.info("Cleaned Dict config")
            else:
                clean_config[key] = str(value)
                logger.info("Cleaned Other config")
    ...
    def prepare_trackers(self) -> None:
        ...
        config_dict = json.loads(self.args.model_dump_json())

        clean_config = self.clean_config_for_tracker(config_dict)
        self.accelerator.init_trackers(tracker_name, config=clean_config)
        ...
        for epoch in range(first_epoch, self.args.train_epochs):
            ...
            for step, batch in enumerate(self.data_loader):
                ...
                with accelerator.accumulate(models_to_accumulate):
                    ...
                    # loss, diff_loss, weighted_vrepa_loss = self.compute_loss(batch)
                ...
                # logs["weighted_vrepa_loss"] = weighted_vrepa_loss.detach().item()
