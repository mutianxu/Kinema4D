...
Encode raw videos, point maps, and their pre-masked variants into VAE
latents, and extract CLIP image embeddings from the first frame.

Pipeline position::

    images_to_video.py ¡ú sam3_video_segmentation.py ¡ú **encode_latents.py**

Expected input layout::

    data_dir/
    ©À©¤©¤ videos/           <n>.mp4   ¡ª original RGB videos
    ©À©¤©¤ pointmap/         <n>.mp4   ¡ª point-map videos
    ©À©¤©¤ mask_videos/      <n>.mp4   ¡ª masked (foreground-only) RGB videos
    ©¸©¤©¤ mask_pointmap/    <n>.mp4   ¡ª masked point-map videos

For every sample the script produces **four** output files:

    1. ``video_latents/<n>.safetensors``
       ¡ª VAE latent of the original video **+ CLIP embedding** of frame 0.
    2. ``pointmap_latents/<n>.pt``
       ¡ª VAE latent of the point-map video.
    3. ``mask_video_latents/<n>.safetensors``
       ¡ª VAE latent of the masked RGB video.
    4. ``mask_pointmap_latents/<n>.pt``
       ¡ª VAE latent of the masked point-map video.
...
def pad_or_crop_temporal(frames: torch.Tensor, target_len: int) -> torch.Tensor:
    ...
    - Too short ¡ú mirror-pad by appending reversed frames.
    - Too long  ¡ú centre-crop along the temporal axis.
    """
    cur = frames.shape[0]
    if cur == target_len:
        return frames
...
def resize_centre_crop(
    ...
    mode : str
        Interpolation mode ¡ª ``"bilinear"`` for RGB, ``"nearest"`` for
        point maps.
    """
...
def preprocess_video(
    ...
    """Read ¡ú resize/crop ¡ú temporal pad/crop.  Convenience one-liner."""
...
def process_one_sample(
    ...
    Output files
    -------------
    1. ``video_latents/<n>.safetensors``        ¡ª VAE latent + CLIP embedding
    2. ``pointmap_latents/<n>.pt``              ¡ª VAE latent
    3. ``mask_video_latents/<n>.safetensors``   ¡ª VAE latent (masked video)
    4. ``mask_pointmap_latents/<n>.pt``         ¡ª VAE latent (masked pointmap)
    """
    paths = {
